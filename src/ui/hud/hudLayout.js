...
export function getHudLayout(screenW, screenH, isTouch = false) {
    // Tweak: minimum scale bumped up so it's readable on small screens
    let S = Math.max(1.8, Math.min(screenW, screenH) / 400);

    // Scale down UI on touch devices so it doesn't cover too much screen
    if (isTouch) {
        S *= 0.50;
    }
